...
import { ExtendedKPIData, RegionName, IndustryCategory } from '@/types/filters';
...
export function ExtendedKPICards({ data, showPercentages = false }: ExtendedKPICardsProps) {
  const [displayMode, setDisplayMode] = useState<'absolute' | 'percent'>('absolute');

  const formatValue = (value: number, isPercent = false) => {
    if (displayMode === 'percent' || isPercent) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <div className="inline-flex items-center gap-1 p-1 bg-muted rounded-lg">
          <Button
            ...
            className={cn(
              'h-7 px-3 text-xs',
              displayMode === 'absolute' && 'bg-card shadow-sm'
            )}
          >
          ...
          <Button
            ...
            className={cn(
              'h-7 px-3 text-xs',
              displayMode === 'percent' && 'bg-card shadow-sm'
            )}
          >
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-primary via-primary to-chart-purple rounded-3xl p-8 text-primary-foreground shadow-2xl">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            ...
            <p className="text-6xl font-black mt-2 tracking-tight drop-shadow-lg">
              {data.totalPeople.toLocaleString()}
            </p>
            <p className="text-sm mt-2 opacity-70">
              Across {data.statesIncluded} states • {data.roleCoverage} roles selected
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard
          ...
          value={data.statesIncluded}
          icon={MapPin}
          variant="teal"
        />
        <KPICard
          title="Regions Included"
          value={data.regionsIncluded}
          icon={Globe2}
          variant="purple"
          subtitle={`of 4 regions`}
        />
        <KPICard
          title="Avg per State"
          value={data.avgPeoplePerState}
          icon={BarChart3}
          variant="amber"
        />
        <KPICard
          title="Role Coverage"
          value={data.roleCoverage}
          icon={Briefcase}
          variant="rose"
          subtitle="roles selected"
        />
      </div>

      {/* Top/Bottom States & Roles */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {data.topStateByPeople && (
          <HighlightCard
            title="Top State"
            primary={US_STATES[data.topStateByPeople.state] || data.topStateByPeople.state}
            secondary={`${data.topStateByPeople.count.toLocaleString()} people`}
            icon={<TrendingUp className="w-4 h-4 text-chart-emerald" />}
            accentColor="emerald"
          />
        )}
        {data.bottomStateByPeople && (
          <HighlightCard
            title="Bottom State"
            primary={US_STATES[data.bottomStateByPeople.state] || data.bottomStateByPeople.state}
            secondary={`${data.bottomStateByPeople.count.toLocaleString()} people`}
            icon={<TrendingDown className="w-4 h-4 text-chart-rose" />}
            accentColor="rose"
          />
        )}
        {data.topRoleByPeople && (
          <HighlightCard
            title="Top Role"
            primary={truncate(data.topRoleByPeople.role, 25)}
            secondary={`${data.topRoleByPeople.count.toLocaleString()} people`}
            icon={<Briefcase className="w-4 h-4 text-chart-purple" />}
            accentColor="purple"
          />
        )}
        {data.topIndustryByPeople && (
          <HighlightCard
            ...
            icon={<Factory className="w-4 h-4 text-chart-amber" />}
            accentColor="amber"
          />
        )}
      </div>

      {/* Industry & Region Breakdown Mini Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Region Breakdown */}
        <div className="chart-container">
          <h3 className="text-sm font-bold text-foreground mb-3">Region Breakdown</h3>
          <div className="space-y-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div 
                          className={cn(
                            "h-full rounded-full transition-all",
                            region === 'Northeast' && "bg-chart-teal",
                            region === 'Midwest' && "bg-chart-purple",
                            region === 'South' && "bg-chart-rose",
                            region === 'West' && "bg-chart-amber"
                          )}
                          style={{ width: `${percent}%` }}
                        />
                      </div>
...
function KPICard({ title, value, icon: Icon, variant, subtitle }: KPICardProps) {
  ...
  return (
    <div className={cn(
      "relative overflow-hidden rounded-2xl p-4 bg-gradient-to-br shadow-lg",
...
function HighlightCard({ title, primary, secondary, icon, accentColor }: HighlightCardProps) {
  ...
  return (
    <div className={cn(
      "p-4 rounded-xl bg-card border border-border border-l-4 shadow-sm",
