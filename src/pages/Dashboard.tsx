...
const Dashboard = () => {
  ...
  // Use global filters hook with geography type
  const {
  ...
  const activeFilterLabels = useMemo(() => {
    const labels: string[] = [];
    if (globalFilters.states.length > 0) labels.push(`${globalFilters.states.length} ${profile.locationLabel?.toLowerCase() || 'states'}`);
    ...
    <div className="dashboard-layout flex flex-col min-h-screen">
      ...
      <header className="bg-card border-b border-border px-6 py-4 shrink-0">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <ICPConfigDialog config={icpConfig} onConfigChange={setICPConfig} columns={activeDataset.columns} />
            <label className="cursor-pointer">
              <input type="file" accept=".xlsx,.xls" className="hidden"
                onChange={async (e) => { const file = e.target.files?.[0]; if (file) await uploadFile(file); e.target.value = ''; }} />
              <Button variant="outline" size="sm" asChild><span><Upload className="w-4 h-4 mr-2" /> New File</span></Button>
            </label>
      ...
      <main className="flex-1 overflow-y-auto scrollbar-thin">
        <div className="dashboard-main">
          <section className="mb-6">
            <ExtendedKPICards data={extendedKPIs} profile={profile} />
          </section>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
            <div className="flex items-center justify-between gap-4 mb-4">
              <TabsList className="bg-muted/50">
                ...
                <TabsTrigger value="charts" className="gap-2"><PieChart className="w-4 h-4" /> Charts</TabsTrigger>
                {profile.mapType !== 'none' && (
                  <TabsTrigger value="map" className="gap-2"><Map className="w-4 h-4" /> Geographic</TabsTrigger>
                )}
                <TabsTrigger value="tables" className="gap-2"><Table className="w-4 h-4" /> Tables</TabsTrigger>
                <TabsTrigger value="ai-insights" className="gap-2"><Brain className="w-4 h-4" /> AI Insights</TabsTrigger>
              </TabsList>
